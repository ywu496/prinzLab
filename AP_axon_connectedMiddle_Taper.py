...
from integration_taper import Integration
...
def gatingUpdate(axo, i):
    ...
    axo.gNa[i] = axo.g_mem_Na_comp[i] * np.power(axo.mNa[i], 2) * axo.hNa[i]

    # Kd
    axo.nKd[i] = nKdUpdate(axo.V_old[i], axo.nKd_old[i])
    axo.gKd[i] = axo.g_mem_Kd_comp[i] * np.power(axo.nKd[i], 4)
    # leak
    axo.gleak[i] = axo.g_mem_leak_comp[i] 
    
    axo.mA[i] = mAUpdate(axo.V_old[i], axo.mA_old[i])
    axo.hA[i] = hAUpdate(axo.V_old[i], axo.hA_old[i])
    axo.gA[i] = axo.g_mem_A_comp[i] * np.power(axo.mA[i], 3) * axo.hA[i] 
    
    axo.mH[i] = mHUpdate(axo.V_old[i], axo.mH_old[i])
    axo.gH[i] = axo.g_mem_H_comp[i] * axo.mH[i]
    
    axo.mM[i] = mMUpdate(axo.V_old[i], axo.mM_old[i])
    axo.gM[i] = axo.g_mem_M_comp[i] * np.power(axo.mM[i], 2)
    
    axo.mCaL[i] = mCaLUpdate(axo.V_old[i], axo.mCaL_old[i])
    axo.hCaL[i] = hCaLUpdate(axo.V_old[i], axo.hCaL_old[i])
    axo.gCaL[i] = axo.g_mem_CaL_comp[i] * axo.mCaL[i] * axo.hCaL[i]
    
    axo.mKCa[i] = mKCaUpdate(axo.mKCa_old[i], axo.CaS_old[i])
    axo.gKCa[i] = axo.g_mem_KCa_comp[i] * axo.mKCa[i]
    
    axo.CaS[i] = CaSUpdate(axo.CaS_old[i], axo.gCaL[i] * (axo.V_old[i] - axo.E_CaL), axo.comp_vol[i]) #
...
while index2 < 1:
    ...
    while index1 < 1:
        ...
        b2d_all = np.linspace(b2d_start, b2d_start * b2_endStartratio, b2_n_comp)
        ...
        b2 = axBranch(L=b2_L, d = b2d_all, l_comp =b1_l_comp , R_ax = 100.0, V_init = -65.0, \
                     mNa_init = mNa_init, hNa_init = hNa_init, nKd_init =  nKd_init, mA_init = mA_init, hA_init = hA_init, mH_init = mH_init, \
                     mM_init = mM_init, mCaL_init = mCaL_init, hCaL_init = hCaL_init, mKCa_init = mKCa_init, CaS_init = CaS_init, \
                     G_leak_abs_uniform = G_leak_abs_uniform, E_leak_uniform = E_leak_uniform, G_Na_abs_uniform = G_Na_abs_uniform, \
                     E_Na_uniform = E_Na_uniform, G_Kd_abs_uniform = G_Kd_abs_uniform, E_K_uniform = E_K_uniform, \
...
#--------
        ...
        b1.ACArr = [([(0.0,0.0)],[(b1.g_cross_comp[0][1], b1.V[1])])]
        

        for i in range(1, b1.n_comp-1, 1): #initialize compartment integration parameter arrays, for middle compartments
            if i == b1.connIndex:
                b1.ACArr.append(([(b1.g_cross_comp[i][0], b1.V[i-1])], [(b1.g_cross_comp[i][1],b1.V[i+1]), (g_middle_junction, b2.V[b2.connIndex])]))
                
            else:
                b1.ACArr.append(([(b1.g_cross_comp[i][0], b1.V[i-1])], [(b1.g_cross_comp[i][1],b1.V[i+1])]))


        b1.ACArr.append(([(b1.g_cross_comp[b1.n_comp-1][0], b1.V[b1.n_comp-2])], [(0.0,0.0)]))
        ...
        b1.V_rec_five = [b1.V_init] #recorded voltage time course in second compartment in mV
        b1.V_rec_12 = [b1.V_init]
        b1.V_rec_14 = [b1.V_init]
        b1.V_rec_16 = [b1.V_init]
        b1.V_rec_18 = [b1.V_init]
        ...
        b2.ACArr = [([(0.0,0.0)],[(b2.g_cross_comp[0][1], b2.V[1])])]
        
        
        for i in range(1, b2.n_comp-1, 1): #initialize compartment integration parameter arrays, for middle compartments
            if i == b2.connIndex:
                b2.ACArr.append(([(b2.g_cross_comp[i][0], b2.V[i-1])], [(b2.g_cross_comp[i][1],b2.V[i+1]), (g_middle_junction, b1.V[b1.connIndex])]))
            else:
                b2.ACArr.append(([(b2.g_cross_comp[i][0], b2.V[i-1])], [(b2.g_cross_comp[i][1],b2.V[i+1])]))
        
        
        
        b2.ACArr.append(([(b2.g_cross_comp[b2.n_comp-1][0], b2.V[b2.n_comp-2])], [(0.0,0.0)]))
        ...
        b2.V_rec_five = [b2.V_init] #recorded voltage time course in second compartment in mV
        ...
        # BEGIN SIMULATION
        slotList = fireSlots(5, 28, 1/7, 0.6)         #original1 
        #slotList = fireSlots(5, 22, 1/5, 0.6)
        #used 2
        while t_now+dt < t_total: 
             ...
             ######################
                ...
                b1.V_rec_five.append(b1.V[4])
                b1.V_rec_12.append(b1.V[11])
                b1.V_rec_14.append(b1.V[13])
                b1.V_rec_16.append(b1.V[15])
                b1.V_rec_18.append(b1.V[17])

                b1.V_rec_last.append(b1.V[-1]) #record voltage in last compartment    
        
                b2.V_rec_first.append(b2.V[1]) #record voltage in first compartment
                b2.V_rec_five.append(b2.V[b2.connIndex-20])
                b2.V_rec_middle.append(b2.V[b2.connIndex]) #record voltage in middle compartment
                b2.V_rec_last.append(b2.V[b2.connIndex+20]) #record voltage in last compartment
...
plotVgeneral(t, b1.V_rec_first, 'b1_V_first', t_total, 231) #Comment out if you don't want to output the graph
plotVgeneral(t, b1.V_rec_middle, 'b1_V_middle(connected)', t_total,232) #Comment out if you don't want to output the graph
plotVgeneral(t, b1.V_rec_last, 'b1_V_last', t_total, 233) #Comment out if you don't want to output the graph
plotVgeneral(t, b2.V_rec_first, 'b2_V_first', t_total, 234)
plotVgeneral(t, b2.V_rec_middle, 'b2_V_middle(connected)', t_total, 235) #Comment out if you don't want to output the graph
plotVgeneral(t, b2.V_rec_last, 'b2_V_last', t_total, 236) #Comment out if you don't want to output the graph
...
plt.show()
        ...
        plt.figure(figsize=(15,12))
        plotVgeneral(t, b1.V_rec_first, 'b1_V_first', t_total, 241) #Comment out if you don't want to output the graph
        plotVgeneral(t, b1.V_rec_five, 'b1_V_5', t_total, 242) #Comment out if you don't want to output the graph
        plotVgeneral(t, b1.V_rec_middle, 'b1_V_middle(connected)', t_total, 243) #Comment out if you don't want to output the graph
        plotVgeneral(t, b1.V_rec_12, 'b1_V_12', t_total, 244) #Comment out if you don't want to output the graph
        plotVgeneral(t, b1.V_rec_14, 'b1_V_14', t_total, 245) #Comment out if you don't want to output the graph
        plotVgeneral(t, b1.V_rec_16, 'b1_V_16', t_total, 246) #Comment out if you don't want to output the graph
        plotVgeneral(t, b1.V_rec_18, 'b1_V_18', t_total, 247) #Comment out if you don't want to output the graph
        plotVgeneral(t, b1.V_rec_last, 'b1_V_last', t_total, 248) #Comment out if you don't want to output the graph
        
        #plt.suptitle(f'b2_diameter: {b2d_ini + index2 * 0.03}')
        plt.suptitle(f'b1_taper(comp_last_d/comp_first_d): {  b1_endStartratio}')

        plt.show()
        
        plt.figure(figsize=(15,12))
        plotVgeneral(t, b1.V_rec_first, 'b1_V_first', t_total, 241) #Comment out if you don't want to output the graph
        plotVgeneral(t, b1.V_rec_five, 'b1_V_five', t_total, 242) #Comment out if you don't want to output the graph
        plotVgeneral(t, b1.V_rec_middle, 'b1_V_middle(connected)', t_total, 243) #Comment out if you don't want to output the graph
        plotVgeneral(t, b1.V_rec_last, 'b1_V_last', t_total, 244) #Comment out if you don't want to output the graph
        plotVgeneral(t, b2.V_rec_first, 'b2_V_2', t_total, 245)
        plotVgeneral(t, b2.V_rec_five, 'b2_V_middle-4', t_total, 246) #Comment out if you don't want to output the graph
        plotVgeneral(t, b2.V_rec_middle, 'b2_V_middle(connected)', t_total, 247) #Comment out if you don't want to output the graph
        plotVgeneral(t, b2.V_rec_last, 'b2_V_middle+4', t_total, 248) #Comment out if you don't want to output the graph
        #plt.suptitle(f'b2_diameter: {b2d_ini + index2 * 0.03}')
        plt.suptitle(f'b1_taper(comp_last_d/comp_first_d): {  b1_endStartratio}')
        ...
        plt.figure(figsize=(15,12))
        plotVgeneral(t, b1.V_rec_first, 'b1_V_first (indexed 0)', t_total, 231) #Comment out if you don't want to output the graph
        plotVgeneral(t, b1.V_rec_middle, 'b1_V_middle (indexed 50)', t_total,232) #Comment out if you don't want to output the graph
        plotVgeneral(t, b1.V_rec_last, 'b1_V_last (indexed 99)', t_total, 233) #Comment out if you don't want to output the graph
        plotVgeneral(t, b2.V_rec_five, 'b1_V_indexed 40', t_total, 234)
        plotVgeneral(t, b2.V_rec_middle, 'b2_V_middle (indexed 50)', t_total, 235) #Comment out if you don't want to output the graph
        plotVgeneral(t, b2.V_rec_last, 'b1_V_indexed 60', t_total, 236) #Comment out if you don't want to output the graph
        #plt.suptitle(f'no gap junction conductance, b1 taper: 0.01')
        
        #plt.suptitle(f'b1 taper: {b1_endStartratio}, b2 taper: {b2_endStartratio}\nb2_first_diameter: {b2d_ini + index2 * 0.03}\nb2_last_diameter:{(b2d_ini + index2 * 0.03) * b2_endStartratio}')
        plt.show()
