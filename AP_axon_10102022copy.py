...
while index2 < 1:
    ...
    while index1 < 1:
        ...
        b1_First = segmentSpikeTest( -30 )
        b1_Middle = segmentSpikeTest( -30 )
        b1_Last = segmentSpikeTest( -30 )
        b2_First  = segmentSpikeTest( -30 )
        b2_Middle  = segmentSpikeTest( -30 )
        b2_Last = segmentSpikeTest( -30 )
        ...
        # branch 1
        b1_L = 200.0 #length of axon in um, distance between sympathetic ganglia is about 1mm
        b1_d = b1d_ini + index1 * -0.03 #diameter of axon in um
        b1_l_comp = 10.0 #length of each compartment in um
        b1_n_comp = int(b1_L / b1_l_comp) #number of compartments
        # branch2
        b2_L = 200.0 #length of axon in um, distance between sympathetic ganglia is about 1mm
        b2_d = b2d_ini + index2 * 0.03 #diameter of axon in um
        b2_l_comp = 10.0 #length of each compartment in um
        b2_n_comp = int(b2_L / b2_l_comp) #number of compartments
        
        #volumes
        comp_vol = np.pi * ((d/2) ** 2) * l_comp #um^3
        b1_comp_vol = np.pi * ((b1_d/2) ** 2) * b1_l_comp #um^3
        b2_comp_vol = np.pi * ((b2_d/2) ** 2) * b2_l_comp #um^3
        #print(f'diameter (um) {d}')
        #print(f"comp_vol(um^3): {comp_vol}")
        #print(f"b1_comp_vol(um^3): {comp_vol}")
        #print(f"b2_comp_vol(um^3): {comp_vol}")

        #IcaLX = -6
        dtX = 0.01
        
        conversion_f = dtX * 1e6 / (comp_vol * 2 * 96485)
        print(f"main compartment: conversion factor: {conversion_f}")

        conversion_f = dtX * 1e6 / (b1_comp_vol * 2 * 96485)
        print(f"b1 compartment: conversion factor: {conversion_f}")

        conversion_f = dtX * 1e6 / (b2_comp_vol * 2 * 96485)

        print(f"b2 compartment: conversion factor: {conversion_f}")
        ...
        b1_G_A_abs = G_A_abs_uniform #default 50 nS
        b1_g_mem_A_comp = b1_conductance_scaling_factor*b1_G_A_abs/1e3
        b1_E_A = E_A_uniform#  default of -90.0 in mV
        
        
        b1_G_H_abs = G_H_abs_uniform #default 1nS
        b1_g_mem_H_comp = b1_conductance_scaling_factor*b1_G_H_abs/1e3
        b1_E_H = E_H_uniform   #  default of -32.0 in mV
        
        
        b1_G_M_abs = G_M_abs_uniform #default 50 nS
        b1_g_mem_M_comp = b1_conductance_scaling_factor*b1_G_M_abs/1e3
        b1_E_M = E_M_uniform#  default of -90.0 in mV
        
        b1_G_CaL_abs = G_CaL_abs_uniform #default 1.2 nS
        b1_g_mem_CaL_comp = b1_conductance_scaling_factor*b1_G_CaL_abs/1e3
        b1_E_CaL = E_CaL_uniform# default of 120.0 in mV
        ###########
        
        b1_G_KCa_abs = G_KCa_abs_uniform #default 50 nS
        b1_g_mem_KCa_comp = b1_conductance_scaling_factor*b1_G_KCa_abs/1e3
        b1_E_KCa = E_KCa_uniform# at default already in mV
        ...
        b2_G_A_abs = G_A_abs_uniform #default 50 nS
        b2_g_mem_A_comp = b2_conductance_scaling_factor*b2_G_A_abs/1e3
        b2_E_A = E_A_uniform#  default of -90.0 in mV
        
        
        b2_G_H_abs =  G_H_abs_uniform#default 1nS
        b2_g_mem_H_comp = b2_conductance_scaling_factor*b2_G_H_abs/1e3
        b2_E_H = E_H_uniform   #  default of -32.0 in mV
        
        
        b2_G_M_abs = G_M_abs_uniform #default 50 nS
        b2_g_mem_M_comp = b2_conductance_scaling_factor*b2_G_M_abs/1e3
        b2_E_M = E_M_uniform#  default of -90.0 in mV
        
        b2_G_CaL_abs = G_CaL_abs_uniform #default 1.2 nS
        b2_g_mem_CaL_comp = b2_conductance_scaling_factor*b2_G_CaL_abs/1e3
        b2_E_CaL = E_CaL_uniform# default of 120.0 in mV
        ###########
        
        b2_G_KCa_abs = G_KCa_abs_uniform #default 50 nS
        b2_g_mem_KCa_comp = b2_conductance_scaling_factor*b2_G_KCa_abs/1e3
        b2_E_KCa = E_KCa_uniform# at default already in mV
        ...
        for i in range(1, n_comp):
            ...
            IH.append(IH_init)
            IM.append(IM_init)
            ICaL.append(ICaL_init)
            IKCa.append(IKCa_init)
            """
        ...
        b1_gA_init = b1_g_mem_A_comp * np.power(b1_mA_init, 3) * b1_hA_init 
        b1_gH_init = b1_g_mem_H_comp * b1_mH_init
        b1_gM_init = b1_g_mem_M_comp * np.power(b1_mM_init, 2)
        b1_gCaL_init = b1_g_mem_CaL_comp * b1_mCaL_init * b1_hCaL_init
        b1_gKCa_init = b1_g_mem_KCa_comp * b1_mKCa_init 
        ...
        b1_IA_init = b1_gA_init * (b1_V_init - b1_E_A)
        b1_IH_init = b1_gH_init * (b1_V_init - b1_E_H)
        b1_IM_init = b1_gM_init * (b1_V_init - b1_E_M)
        b1_ICaL_init = b1_gCaL_init * (b1_V_init - b1_E_CaL)
        b1_IKCa_init = b1_gKCa_init * (b1_V_init - b1_E_KCa)
        """
        ...
        b1_mH = [b1_mH_init]
        b1_mM = [b1_mM_init]
        b1_mCaL = [b1_mCaL_init]
        b1_hCaL = [b1_hCaL_init]
        b1_mKCa= [b1_mKCa_init]
        b1_CaS = [b1_CaS_init]
        ...
        b1_gH = [b1_gH_init]
        b1_gM = [b1_gM_init]
        b1_gCaL = [b1_gCaL_init]
        b1_gKCa = [b1_gKCa_init]
        ...
        b1_IH = [b1_IH_init]
        b1_IM = [b1_IM_init]
        b1_ICaL = [b1_ICaL_init]
        b1_IKCa = [b1_IKCa_init]
        """
        ...
        for i in range(1, b1_n_comp):
            ...
            b1_mA.append(b1_mA_init) #initialize compartment Kd activation array
            b1_hA.append(b1_hA_init) #initialize compartment Kd activation array
            b1_mH.append(b1_mH_init)
            b1_mCaL.append(b1_mCaL_init)
            b1_hCaL.append(b1_hCaL_init)
            b1_mM.append(b1_mM_init)
            b1_mKCa.append(b1_mKCa_init)
            b1_CaS.append(b1_CaS_init)
            ...
            b1_gA.append(b1_gA_init)
            b1_gH.append(b1_gH_init)
            b1_gM.append(b1_gM_init)
            b1_gCaL.append(b1_gCaL_init)
            b1_gKCa.append(b1_gKCa_init)
            ...
            b1_IA.append(b1_IA_init)
            b1_IH.append(b1_IH_init)
            b1_IM.append(b1_IM_init)
            b1_ICaL.append(b1_ICaL_init)
            b1_IKCa.append(b1_IKCa_init)
            """
        ...
        b1_mA_old = b1_mA.copy()
        b1_mM_old = b1_mM.copy()
        b1_hA_old = b1_hA.copy()
        b1_mH_old = b1_mH.copy()
        b1_mCaL_old = b1_mCaL.copy()
        b1_hCaL_old = b1_hCaL.copy()
        b1_mKCa_old = b1_mKCa.copy()
        b1_CaS_old = b1_CaS.copy()
        ...
        b1_gA_old = b1_gA.copy()
        b1_gH_old = b1_gH.copy()
        b1_gM_old = b1_gM.copy()
        b1_gCaL_old = b1_gCaL.copy()
        b1_gKCa_old = b1_gKCa.copy()
        ...
        b1_IA_old = b1_IA.copy()
        b1_IH_old = b1_IH.copy()
        b1_IM_old = b1_IM.copy()
        b1_ICaL _old = b1 _ICaL.copy()
        b1_IKCa_old = b1_IKCa.copy()
        """
        #branch b2
        ...
        b2_gA_init = b2_g_mem_A_comp * np.power(b2_mA_init, 3) * b2_hA_init 
        b2_gH_init = b2_g_mem_H_comp * b2_mH_init
        b2_gM_init = b2_g_mem_M_comp * np.power(b2_mM_init, 2)
        b2_gCaL_init = b2_g_mem_CaL_comp * b2_mCaL_init * b2_hCaL_init
        b2_gKCa_init = b2_g_mem_KCa_comp * b2_mKCa_init 
        ...
        b2_IA_init = b2_gA_init * (b2_V_init - b2_E_A)
        b2_IH_init = b2_gH_init * (b2_V_init - b2_E_H)
        b2_IM_init = b2_gM_init * (b2_V_init - b2_E_M)
        b2_ICaL_init = b2_gCaL_init * (b2_V_init - b2_E_CaL)
        b2_IKCa_init = b2_gKCa_init * (b2_V_init - b2_E_KCa)
        """
        ...
        b2_mH = [b2_mH_init]
        b2_mM = [b2_mM_init]
        b2_mCaL = [b2_mCaL_init]
        b2_hCaL = [b2_hCaL_init]
        b2_mKCa= [b2_mKCa_init]
        b2_CaS = [b2_CaS_init]
        ...
        b2_gH = [b2_gH_init]
        b2_gM = [b2_gM_init]
        b2_gCaL = [b2_gCaL_init]
        b2_gKCa = [b2_gKCa_init]
        ...
        b2_IH = [b2_IH_init]
        b2_IM = [b2_IM_init]
        b2_ICaL = [b2_ICaL_init]
        b2_IKCa = [b2_IKCa_init]
        """
        for i in range(1, b2_n_comp):
            ...
            b2_mA.append(b2_mA_init) #initialize compartment Kd activation array
            b2_hA.append(b2_hA_init) #initialize compartment Kd activation array
            b2_mH.append(b2_mH_init)
            b2_mCaL.append(b2_mCaL_init)
            b2_hCaL.append(b2_hCaL_init)
            b2_mM.append(b2_mM_init)
            b2_mKCa.append(b2_mKCa_init)
            b2_CaS.append(b2_CaS_init)
            ...
            b2_gA.append(b2_gA_init)
            b2_gH.append(b2_gH_init)
            b2_gM.append(b2_gM_init)
            b2_gCaL.append(b2_gCaL_init)
            b2_gKCa.append(b2_gKCa_init)
            ...
            b2_IA.append(b2_IA_init)
            b2_IH.append(b2_IH_init)
            b2_IM.append(b2_IM_init)
            b2_ICaL.append(b2_ICaL_init)
            b2_IKCa.append(b2_IKCa_init)
            """
            
        b2_gGABA[b2_n_comp-2] = b2_g_mem_GABA #put GABA conductance only in compartment proximal to branch point (not at branch point)
        
        #These parts seem to be unnecessary
        """
        # store previous time step values in _old arrays
        ...
        b2_mA_old = b2_mA.copy()
        b2_mM_old = b2_mM.copy()
        b2_hA_old = b2_hA.copy()
        b2_mH_old = b2_mH.copy()
        b2_mCaL_old = b2_mCaL.copy()
        b2_hCaL_old = b2_hCaL.copy()
        b2_mKCa_old = b2_mKCa.copy()
        b2_CaS_old = b2_CaS.copy()
        ...
        b2_gA_old = b2_gA.copy()
        b2_gH_old = b2_gH.copy()
        b2_gM_old = b2_gM.copy()
        b2_gCaL_old = b2_gCaL.copy()
        b2_gKCa_old = b2_gKCa.copy()
        ...
        b2_IA_old = b2_IA.copy()
        b2_IH_old = b2_IH.copy()
        b2_IM_old = b2_IM.copy()
        b2_ICaL _old = b2_ICaL .copy()
        b2_IKCa_old = b2_IKCa.copy()
        """
        ...
        b1_B = [-(b1_gNa[0]+b1_gKd[0]+b1_gleak[0]+b1_gGABA[0]+b1_gA[0] + b1_gH[0] + b1_gM[0] + b1_gCaL[0] + b1_gKCa[0]+b1_g_ax_comp)/b1_C_mem_comp] #in units of uS/nF
        b1_C = [b1_g_ax_comp/b1_C_mem_comp] #in units of uS/nF
        b1_D = [(b1_gNa[0]*b1_E_Na+b1_gKd[0]*b1_E_K+b1_gleak[0]*b1_E_leak+b1_gGABA[0]*b1_E_GABA
                 +b1_gA[0]*b1_E_A + b1_gH[0]*b1_E_H + b1_gM[0]*b1_E_M + b1_gCaL[0]*b1_E_CaL + b1_gKCa[0]*b1_E_KCa)/b1_C_mem_comp] #in units of nA/nF
        ...
        for i in range(1, b1_n_comp-1, 1): #initialize compartment integration parameter arrays, for middle compartments
            ...
            b1_B.append(-(b1_gNa[i]+b1_gKd[i]+b1_gleak[i]+b1_gGABA[i]+b1_gA[i] + b1_gH[i] + b1_gM[i] + b1_gCaL[i] + b1_gKCa[i]+2.0*b1_g_ax_comp)/b1_C_mem_comp) #in units of uS/nF
            b1_C.append(b1_g_ax_comp/b1_C_mem_comp) #in units of uS/nF
            b1_D.append((b1_gNa[i]*b1_E_Na+b1_gKd[i]*b1_E_K+b1_gleak[i]*b1_E_leak+b1_gGABA[i]*b1_E_GABA
                         + b1_gA[i]*b1_E_A + b1_gH[i]*b1_E_H + b1_gM[i]*b1_E_M + b1_gCaL[i]*b1_E_CaL + b1_gKCa[i]*b1_E_KCa)/b1_C_mem_comp) #in units of nA/nF
        ...
        b1_B.append(-(b1_gNa[b1_n_comp-1]+b1_gKd[b1_n_comp-1]+b1_gleak[b1_n_comp-1]+b1_gGABA[b1_n_comp-1]+b1_g_ax_comp
                      +b1_gA[b1_n_comp-1] + b1_gH[b1_n_comp-1] + b1_gM[b1_n_comp-1] + b1_gCaL[b1_n_comp-1] + b1_gKCa[b1_n_comp-1]+g_main_b1)/b1_C_mem_comp) #in units of uS/nF
        b1_C.append(g_main_b1/b1_C_mem_comp) #in units of uS/nF
        b1_D.append((b1_gNa[b1_n_comp-1]*b1_E_Na+b1_gKd[b1_n_comp-1]*b1_E_K+b1_gleak[b1_n_comp-1]*b1_E_leak+b1_gGABA[b1_n_comp-1]*b1_E_GABA
                     +b1_gA[b1_n_comp-1]*b1_E_A + b1_gH[b1_n_comp-1]*b1_E_H + b1_gM[b1_n_comp-1]*b1_E_M + b1_gCaL[b1_n_comp-1]*b1_E_CaL + b1_gKCa[b1_n_comp-1]*b1_E_KCa)/b1_C_mem_comp) #in units of nA/nF
        ...
        b2_B = [-(b2_gNa[0]+b2_gKd[0]+b2_gleak[0]+b2_gGABA[0]+b2_gA[0] + b2_gH[0] + b2_gM[0] + b2_gCaL[0] + b2_gKCa[0]+b2_g_ax_comp)/b2_C_mem_comp] #in units of uS/nF
        b2_C = [b2_g_ax_comp/b2_C_mem_comp] #in units of uS/nF
        b2_D = [(b2_gNa[0]*b2_E_Na+b2_gKd[0]*b2_E_K+b2_gleak[0]*b2_E_leak+b2_gGABA[0]*b2_E_GABA
                 +b2_gA[0]*b2_E_A + b2_gH[0]*b2_E_H + b2_gM[0]*b2_E_M + b2_gCaL[0]*b2_E_CaL + b2_gKCa[0]*b2_E_KCa)/b2_C_mem_comp] #in units of nA/nF
        ...
        for i in range(1, b2_n_comp-1, 1): #initialize compartment integration parameter arrays, for middle compartments
            ...
            b2_B.append(-(b2_gNa[i]+b2_gKd[i]+b2_gleak[i]+b2_gGABA[i]+b2_gA[i] + b2_gH[i] + b2_gM[i] + b2_gCaL[i] + b2_gKCa[i]+2.0*b2_g_ax_comp)/b2_C_mem_comp) #in units of uS/nF
            b2_C.append(b2_g_ax_comp/b2_C_mem_comp) #in units of uS/nF
            b2_D.append((b2_gNa[i]*b2_E_Na+b2_gKd[i]*b2_E_K+b2_gleak[i]*b2_E_leak+b2_gGABA[i]*b2_E_GABA
                         + b2_gA[i]*b2_E_A + b2_gH[i]*b2_E_H + b2_gM[i]*b2_E_M + b2_gCaL[i]*b2_E_CaL + b2_gKCa[i]*b2_E_KCa)/b2_C_mem_comp) #in units of nA/nF
        ...
        b2_B.append(-(b2_gNa[b2_n_comp-1]+b2_gKd[b2_n_comp-1]+b2_gleak[b2_n_comp-1]+b2_gGABA[b2_n_comp-1]+b2_g_ax_comp
                      +b2_gA[b2_n_comp-1] + b2_gH[b2_n_comp-1] + b2_gM[b2_n_comp-1] + b2_gCaL[b2_n_comp-1] + b2_gKCa[b2_n_comp-1]+g_main_b2)/b2_C_mem_comp) #in units of uS/nF
        b2_C.append(g_main_b2/b2_C_mem_comp) #in units of uS/nF
        b2_D.append((b2_gNa[b2_n_comp-1]*b2_E_Na+b2_gKd[b2_n_comp-1]*b2_E_K+b2_gleak[b2_n_comp-1]*b2_E_leak+b2_gGABA[b2_n_comp-1]*b2_E_GABA
                      +b2_gA[b2_n_comp-1]*b2_E_A + b2_gH[b2_n_comp-1]*b2_E_H + b2_gM[b2_n_comp-1]*b2_E_M + b2_gCaL[b2_n_comp-1]*b2_E_CaL + b2_gKCa[b2_n_comp-1]*b2_E_KCa)/b2_C_mem_comp) #in units of nA/nF
        ...
        while t_now+dt < t_total: 
               ...
               # if t_now>=t_stimstart and t_now<t_stimend:
                ...
                b1_mNa[0] = mNaUpdate(b1_V_old[0], b1_mNa_old[0])
                b1_hNa[0] = hNaUpdate(b1_V_old[0], b1_hNa_old[0])
                b1_gNa[0] = b1_g_mem_Na_comp * np.power(b1_mNa[0], 2) * b1_hNa[0]
        
                # Kd
                b1_nKd[0] = nKdUpdate(b1_V_old[0], b1_nKd_old[0])
                b1_gKd[0] = b1_g_mem_Kd_comp * np.power(b1_nKd[0], 4)
                # leak
                b1_gleak[0] = b1_g_mem_leak_comp 
                
                b1_mA[0] = mAUpdate(b1_V_old[0], b1_mA_old[0])
                b1_hA[0] = hAUpdate(b1_V_old[0], b1_hA_old[0])
                b1_gA[0] = b1_g_mem_A_comp * np.power(b1_mA[0], 3) * b1_hA[0] 
                
                b1_mH[0] = mHUpdate(b1_V_old[0], b1_mH_old[0])
                b1_gH[0] = b1_g_mem_H_comp * b1_mH[0]
                
                b1_mM[0] = mMUpdate(b1_V_old[0], b1_mM_old[0])
                b1_gM[0] = b1_g_mem_M_comp * np.power(b1_mM[0], 2)
                
                b1_mCaL[0] = mCaLUpdate(b1_V_old[0], b1_mCaL_old[0])
                b1_hCaL[0] = hCaLUpdate(b1_V_old[0], b1_hCaL_old[0])
                b1_gCaL[0] = b1_g_mem_CaL_comp * b1_mCaL[0] * b1_hCaL[0]
                
                b1_mKCa[0] = mKCaUpdate(b1_mKCa_old[0], b1_CaS_old[0])
                b1_gKCa[0] = b1_g_mem_KCa_comp * b1_mKCa[0]
                
                b1_CaS[0] = CaSUpdate(b1_CaS_old[0], b1_gCaL[0] * (b1_V_old[0] - b1_E_CaL), b1_comp_vol) #
                #print (b1_gCaL[0] * (b1_V_old[0] - b1_E_CaL))
                ...
                #####
                b1_B[0] = -(b1_gNa[0]+b1_gKd[0]+b1_gleak[0]+b1_gGABA[0]+b1_gA[0]
                            +b1_gH[0]+b1_gM[0]+b1_gCaL[0]+b1_gKCa[0]+b1_g_ax_comp)/b1_C_mem_comp #in units of uS/nF
                #####
                b1_C[0] = b1_g_ax_comp/b1_C_mem_comp #in units of uS/nF
                #####
                b1_D[0] = (b1_gNa[0]*b1_E_Na+b1_gKd[0]*b1_E_K+b1_gleak[0]*b1_E_leak+b1_gGABA[0]*b1_E_GABA
                           + b1_gA[0]*b1_E_A + b1_gH[0]*b1_E_H + b1_gM[0]*b1_E_M + b1_gCaL[0]*b1_E_CaL + b1_gKCa[0]*b1_E_KCa)/b1_C_mem_comp #in units of nA/nF
                #####
                ...
                for i in range(1, b1_n_comp-1, 1): #middle compartments
                    # Na
                    b1_mNa[i] = mNaUpdate(b1_V_old[i], b1_mNa_old[i])
                    b1_hNa[i] = hNaUpdate(b1_V_old[i], b1_hNa_old[i])
                    b1_gNa[i] = b1_g_mem_Na_comp * np.power(b1_mNa[i], 2) * b1_hNa[i]
        
                    # Kd
                    b1_nKd[i] = nKdUpdate(b1_V_old[i], b1_nKd_old[i])
                    b1_gKd[i] = b1_g_mem_Kd_comp * np.power(b1_nKd[i], 4)
        
                    # leak
                    b1_gleak[i] = b1_g_mem_leak_comp 
                    
                    b1_mA[i] = mAUpdate(b1_V_old[i], b1_mA_old[i])
                    b1_hA[i] = hAUpdate(b1_V_old[i], b1_hA_old[i])
                    b1_gA[i] = b1_g_mem_A_comp * np.power(b1_mA[i], 3) * b1_hA[i] 
                    
                    b1_mH[i] = mHUpdate(b1_V_old[i], b1_mH_old[i])
                    b1_gH[i] = b1_g_mem_H_comp * b1_mH[i]
                    
                    b1_mM[i] = mMUpdate(b1_V_old[i], b1_mM_old[i])
                    b1_gM[i] = b1_g_mem_M_comp * np.power(b1_mM[i], 2)
                    
                    b1_mCaL[i] = mCaLUpdate(b1_V_old[i], b1_mCaL_old[i])
                    b1_hCaL[i] = hCaLUpdate(b1_V_old[i], b1_hCaL_old[i])
                    b1_gCaL[i] = b1_g_mem_CaL_comp * b1_mCaL[i] * b1_hCaL[i]
                    
        
                    b1_mKCa[i] = mKCaUpdate(b1_mKCa_old[i], b1_CaS_old[i])
                    b1_gKCa[i] = b1_g_mem_KCa_comp * b1_mKCa[i]
                    b1_CaS[i] = CaSUpdate(b1_CaS_old[i], b1_gCaL[i] * (b1_V_old[i] - b1_E_CaL), b1_comp_vol)
                    ...
                    ######
                    b1_B[i] = -(b1_gNa[i]+b1_gKd[i]+b1_gleak[i]+b1_gGABA[i]+b1_gA[i]
                                +b1_gH[i]+b1_gM[i]+b1_gCaL[i]+b1_gKCa[i]+ 2.0*b1_g_ax_comp)/b1_C_mem_comp
                    ######
                    b1_C[i] = b1_g_ax_comp/b1_C_mem_comp #in units of uS/nF
                    #####
                    b1_D[i] = (b1_gNa[i]*b1_E_Na+b1_gKd[i]*b1_E_K+b1_gleak[i]*b1_E_leak+b1_gGABA[i]*b1_E_GABA
                               + b1_gA[i]*b1_E_A + b1_gH[i]*b1_E_H + b1_gM[i]*b1_E_M + b1_gCaL[i]*b1_E_CaL + b1_gKCa[i]*b1_E_KCa)/b1_C_mem_comp
                    #####
                    b1_a[i] = b1_A[i]*dt    
                    b1_b[i] = b1_B[i]*dt
                    b1_c[i] = b1_C[i]*dt
                ...
                b1_mNa[b1_n_comp-1] = mNaUpdate(b1_V_old[b1_n_comp-1], b1_mNa_old[b1_n_comp-1])
                b1_hNa[b1_n_comp-1] = hNaUpdate(b1_V_old[b1_n_comp-1], b1_hNa_old[b1_n_comp-1])
                b1_gNa[b1_n_comp-1] = b1_g_mem_Na_comp * np.power(b1_mNa[b1_n_comp-1], 2) * b1_hNa[b1_n_comp-1]
        
                # Kd
                b1_nKd[b1_n_comp-1] = nKdUpdate(b1_V_old[b1_n_comp-1], b1_nKd_old[b1_n_comp-1])
                b1_gKd[b1_n_comp-1] = b1_g_mem_Kd_comp * np.power(b1_nKd[b1_n_comp-1], 4)
        
                # leak
                b1_gleak[b1_n_comp-1] = b1_g_mem_leak_comp 
                b1_mA[b1_n_comp-1] = mAUpdate(b1_V_old[b1_n_comp-1], b1_mA_old[b1_n_comp-1])
                b1_hA[b1_n_comp-1] = hAUpdate(b1_V_old[b1_n_comp-1], b1_hA_old[b1_n_comp-1])
                b1_gA[b1_n_comp-1] = b1_g_mem_A_comp * np.power(b1_mA[b1_n_comp-1], 3) * b1_hA[b1_n_comp-1] 
                
                b1_mH[b1_n_comp-1] = mHUpdate(b1_V_old[b1_n_comp-1], b1_mH_old[b1_n_comp-1])
                b1_gH[b1_n_comp-1] = b1_g_mem_H_comp * b1_mH[b1_n_comp-1]
                
                b1_mM[b1_n_comp-1] = mMUpdate(b1_V_old[b1_n_comp-1], b1_mM_old[b1_n_comp-1])
                b1_gM[b1_n_comp-1] = b1_g_mem_M_comp * np.power(b1_mM[b1_n_comp-1], 2)
                
                b1_mCaL[b1_n_comp-1] = mCaLUpdate(b1_V_old[b1_n_comp-1], b1_mCaL_old[b1_n_comp-1])
                b1_hCaL[b1_n_comp-1] = hCaLUpdate(b1_V_old[b1_n_comp-1], b1_hCaL_old[b1_n_comp-1])
                b1_gCaL[b1_n_comp-1] = b1_g_mem_CaL_comp * b1_mCaL[b1_n_comp-1] * b1_hCaL[b1_n_comp-1]
                
                b1_mKCa[b1_n_comp-1] = mKCaUpdate(b1_mKCa_old[b1_n_comp-1], b1_CaS_old[b1_n_comp-1])
                b1_gKCa[b1_n_comp-1] = b1_g_mem_KCa_comp * b1_mKCa[b1_n_comp-1]
                b1_CaS[b1_n_comp-1] = CaSUpdate(b1_CaS_old[b1_n_comp-1], b1_gCaL[b1_n_comp-1] * (b1_V_old[b1_n_comp-1] - b1_E_CaL), b1_comp_vol)
        
                # integration parameters
                
                b1_A[b1_n_comp-1] = b1_g_ax_comp/b1_C_mem_comp #in units of uS/nF
                ####
                b1_B[b1_n_comp-1] = -(b1_gNa[b1_n_comp-1]+b1_gKd[b1_n_comp-1]+b1_gleak[b1_n_comp-1]+b1_gGABA[b1_n_comp-1]
                                      +b1_gA[b1_n_comp-1]+b1_gH[b1_n_comp-1]+b1_gM[b1_n_comp-1]+b1_gCaL[b1_n_comp-1]+b1_gKCa[b1_n_comp-1]
                                      +b1_g_ax_comp+g_main_b1)/b1_C_mem_comp
                #####
                b1_C[b1_n_comp-1] = g_main_b1/b1_C_mem_comp #in units of uS/nF
                #######
                b1_D[b1_n_comp-1] = (b1_gNa[b1_n_comp-1]*b1_E_Na+b1_gKd[b1_n_comp-1]*b1_E_K+b1_gleak[b1_n_comp-1]*b1_E_leak+b1_gGABA[b1_n_comp-1]*b1_E_GABA
                                     + b1_gA[b1_n_comp-1]*b1_E_A + b1_gH[b1_n_comp-1]*b1_E_H + b1_gM[b1_n_comp-1]*b1_E_M + b1_gCaL[b1_n_comp-1]*b1_E_CaL + b1_gKCa[b1_n_comp-1]*b1_E_KCa)/b1_C_mem_comp
                #######
                ...
                b2_mNa[0] = mNaUpdate(b2_V_old[0], b2_mNa_old[0])
                b2_hNa[0] = hNaUpdate(b2_V_old[0], b2_hNa_old[0])
                b2_gNa[0] = b2_g_mem_Na_comp * np.power(b2_mNa[0], 2) * b2_hNa[0]
        
                # Kd
                b2_nKd[0] = nKdUpdate(b2_V_old[0], b2_nKd_old[0])
                ...
                b2_mA[0] = mAUpdate(b2_V_old[0], b2_mA_old[0])
                b2_hA[0] = hAUpdate(b2_V_old[0], b2_hA_old[0])
                b2_gA[0] = b2_g_mem_A_comp * np.power(b2_mA[0], 3) * b2_hA[0] 
                
                b2_mH[0] = mHUpdate(b2_V_old[0], b2_mH_old[0])
                b2_gH[0] = b2_g_mem_H_comp * b2_mH[0]
                
                b2_mM[0] = mMUpdate(b2_V_old[0], b2_mM_old[0])
                b2_gM[0] = b2_g_mem_M_comp * np.power(b2_mM[0], 2)
                
                b2_mCaL[0] = mCaLUpdate(b2_V_old[0], b2_mCaL_old[0])
                b2_hCaL[0] = hCaLUpdate(b2_V_old[0], b2_hCaL_old[0])
                b2_gCaL[0] = b2_g_mem_CaL_comp * b2_mCaL[0] * b2_hCaL[0]
                
                b2_mKCa[0] = mKCaUpdate(b2_mKCa_old[0], b2_CaS_old[0])
                b2_gKCa[0] = b2_g_mem_KCa_comp * b2_mKCa[0]
                b2_CaS[0] = CaSUpdate(b2_CaS_old[0], b2_gCaL[0] * (b2_V_old[0] - b2_E_CaL), b2_comp_vol)
        
        
                # integration parameters
                b2_A[0] = 0.0 #first compartment, in units of uS/nF
                #####
                b2_B[0] = -(b2_gNa[0]+b2_gKd[0]+b2_gleak[0]+b2_gGABA[0]+b2_gA[0]
                            +b2_gH[0]+b2_gM[0]+b2_gCaL[0]+b2_gKCa[0]+b2_g_ax_comp)/b2_C_mem_comp #in units of uS/nF
                ######
                b2_C[0] = b2_g_ax_comp/b2_C_mem_comp #in units of uS/nF
                #####
                b2_D[0] = (b2_gNa[0]*b2_E_Na+b2_gKd[0]*b2_E_K+b2_gleak[0]*b2_E_leak+b2_gGABA[0]*b2_E_GABA
                           + b2_gA[0]*b2_E_A + b2_gH[0]*b2_E_H + b2_gM[0]*b2_E_M + b2_gCaL[0]*b2_E_CaL + b2_gKCa[0]*b2_E_KCa)/b2_C_mem_comp #in units of nA/nF
                #####
                ...
                for i in range(1, b2_n_comp-1, 1): #middle compartments
                    ...
                    b2_mNa[i] = mNaUpdate(b2_V_old[i], b2_mNa_old[i])
                    b2_hNa[i] = hNaUpdate(b2_V_old[i], b2_hNa_old[i])
                    b2_gNa[i] = b2_g_mem_Na_comp * np.power(b2_mNa[i], 2) * b2_hNa[i]
        
                    # Kd
         
                    b2_nKd[i] = nKdUpdate(b2_V_old[i], b2_nKd_old[i])
                    ...
                    b2_mA[i] = mAUpdate(b2_V_old[i], b2_mA_old[i])
                    b2_hA[i] = hAUpdate(b2_V_old[i], b2_hA_old[i])
                    b2_gA[i] = b2_g_mem_A_comp * np.power(b2_mA[i], 3) * b2_hA[i] 
                    
                    b2_mH[i] = mHUpdate(b2_V_old[i], b2_mH_old[i])
                    b2_gH[i] = b2_g_mem_H_comp * b2_mH[i]
                    
                    b2_mM[i] = mMUpdate(b2_V_old[i], b2_mM_old[i])
                    b2_gM[i] = b2_g_mem_M_comp * np.power(b2_mM[i], 2)
                    
                    b2_mCaL[i] = mCaLUpdate(b2_V_old[i], b2_mCaL_old[i])
                    b2_hCaL[i] = hCaLUpdate(b2_V_old[i], b2_hCaL_old[i])
                    b2_gCaL[i] = b2_g_mem_CaL_comp * b2_mCaL[i] * b2_hCaL[i]
        
                    b2_mKCa[i] = mKCaUpdate(b2_mKCa_old[i], b2_CaS_old[i])
                    b2_gKCa[i] = b2_g_mem_KCa_comp * b2_mKCa[i]
                    b2_CaS[i] = CaSUpdate(b2_CaS_old[i], b2_gCaL[i] * (b2_V_old[i] - b2_E_CaL),b2_comp_vol)
                    ...
                    ########
                    b2_B[i] = -(b2_gNa[i]+b2_gKd[i]+b2_gleak[i]+b2_gGABA[i]+b2_gA[i]
                                +b2_gH[i]+b2_gM[i]+b2_gCaL[i]+b2_gKCa[i]+2.0*b2_g_ax_comp)/b2_C_mem_comp
                    #####
                    b2_C[i] = b2_g_ax_comp/b2_C_mem_comp #in units of uS/nF
                    ######
                    b2_D[i] = (b2_gNa[i]*b2_E_Na+b2_gKd[i]*b2_E_K+b2_gleak[i]*b2_E_leak+b2_gGABA[i]*b2_E_GABA
                               + b2_gA[i]*b2_E_A + b2_gH[i]*b2_E_H + b2_gM[i]*b2_E_M + b2_gCaL[i]*b2_E_CaL + b2_gKCa[i]*b2_E_KCa)/b2_C_mem_comp
                    #####
                    b2_a[i] = b2_A[i]*dt    
                    b2_b[i] = b2_B[i]*dt
                    b2_c[i] = b2_C[i]*dt
                ...
                b2_mNa[b2_n_comp-1] = mNaUpdate(b2_V_old[b2_n_comp-1], b2_mNa_old[b2_n_comp-1])
                b2_hNa[b2_n_comp-1] = hNaUpdate(b2_V_old[b2_n_comp-1], b2_hNa_old[b2_n_comp-1])
                b2_gNa[b2_n_comp-1] = b2_g_mem_Na_comp * np.power(b2_mNa[b2_n_comp-1], 2) * b2_hNa[b2_n_comp-1]
        
                # Kd
        
                b2_nKd[b2_n_comp-1] = nKdUpdate(b2_V_old[b2_n_comp-1], b2_nKd_old[b2_n_comp-1])
                ...
                b2_mA[b2_n_comp-1] = mAUpdate(b2_V_old[b2_n_comp-1], b2_mA_old[b2_n_comp-1])
                b2_hA[b2_n_comp-1] = hAUpdate(b2_V_old[b2_n_comp-1], b2_hA_old[b2_n_comp-1])
                b2_gA[b2_n_comp-1] = b2_g_mem_A_comp * np.power(b2_mA[b2_n_comp-1], 3) * b2_hA[b2_n_comp-1] 
                
                b2_mH[b2_n_comp-1] = mHUpdate(b2_V_old[b2_n_comp-1], b2_mH_old[b2_n_comp-1])
                b2_gH[b2_n_comp-1] = b2_g_mem_H_comp * b2_mH[b2_n_comp-1]
                
                b2_mM[b2_n_comp-1] = mMUpdate(b2_V_old[b2_n_comp-1], b2_mM_old[b2_n_comp-1])
                b2_gM[b2_n_comp-1] = b2_g_mem_M_comp * np.power(b2_mM[b2_n_comp-1], 2)
                
                b2_mCaL[b2_n_comp-1] = mCaLUpdate(b2_V_old[b2_n_comp-1], b2_mCaL_old[b2_n_comp-1])
                b2_hCaL[b2_n_comp-1] = hCaLUpdate(b2_V_old[b2_n_comp-1], b2_hCaL_old[b2_n_comp-1])
                b2_gCaL[b2_n_comp-1] = b2_g_mem_CaL_comp * b2_mCaL[b2_n_comp-1] * b2_hCaL[b2_n_comp-1]
                
                b2_mKCa[b2_n_comp-1] = mKCaUpdate(b2_mKCa_old[b2_n_comp-1], b2_CaS_old[b2_n_comp-1])
                b2_gKCa[b2_n_comp-1] = b2_g_mem_KCa_comp * b2_mKCa[b2_n_comp-1]
                b2_CaS[b2_n_comp-1] = CaSUpdate(b2_CaS_old[b2_n_comp-1], b2_gCaL[b2_n_comp-1] * (b2_V_old[b2_n_comp-1] - b2_E_CaL),b2_comp_vol)
        
                # integration parameters
                b2_A[b2_n_comp-1] = b2_g_ax_comp/b2_C_mem_comp #in units of uS/nF
                #######
                b2_B[b2_n_comp-1] = -(b2_gNa[b2_n_comp-1]+b2_gKd[b2_n_comp-1]+b2_gleak[b2_n_comp-1]+b2_gGABA[b2_n_comp-1]
                                      +b2_gA[b2_n_comp-1]+b2_gH[b2_n_comp-1]+b2_gM[b2_n_comp-1]+b2_gCaL[b2_n_comp-1]+b2_gKCa[b2_n_comp-1]
                                      +b2_g_ax_comp+g_main_b2)/b2_C_mem_comp
                #######
                b2_C[b2_n_comp-1] = g_main_b2/b2_C_mem_comp #in units of uS/nF
                #######
                b2_D[b2_n_comp-1] = (b2_gNa[b2_n_comp-1]*b2_E_Na+b2_gKd[b2_n_comp-1]*b2_E_K+b2_gleak[b2_n_comp-1]*b2_E_leak+b2_gGABA[b2_n_comp-1]*b2_E_GABA
                                     + b2_gA[b2_n_comp-1]*b2_E_A + b2_gH[b2_n_comp-1]*b2_E_H + b2_gM[b2_n_comp-1]*b2_E_M + b2_gCaL[b2_n_comp-1]*b2_E_CaL + b2_gKCa[b2_n_comp-1]*b2_E_KCa)/b2_C_mem_comp
                #######
                ...
                b1_First.findSpike(b1_V_rec_first[-1]) 
                b1_Middle.findSpike(b1_V_rec_middle[-1]) 
                b1_Last.findSpike(b1_V_rec_last[-1]) 
                b2_First.findSpike(b2_V_rec_first[-1])  
                b2_Middle.findSpike(b2_V_rec_middle[-1])  
                b2_Last.findSpike(b2_V_rec_last[-1]) 
        ...
        print("b1_First_Spike_Number: %d" % b1_First.Seg_spike_num)
        print("b1_Middle_Spike_Number: %d" % b1_Middle.Seg_spike_num)
        print("b1_Last_Spike_Number: %d" % b1_Last.Seg_spike_num)
        print("b2_First_Spike_Number: %d" % b2_First.Seg_spike_num)
        print("b2_Middle_Spike_Number: %d" % b2_Middle.Seg_spike_num)
        print("b2_Last_Spike_Number: %d" % b2_Last.Seg_spike_num)
        #arrayR = [("mf", main_First.Seg_spike_num), ("mm", main_Middle.Seg_spike_num), ("ml", main_Last.Seg_spike_num), ("b1f", b1_First.Seg_spike_num), ("b1m", b1_Middle.Seg_spike_num),
         #         ("b1l", b1_Last.Seg_spike_num), ("b2f", b2_First.Seg_spike_num), ("b2m", b2_Middle.Seg_spike_num), ("b2l", b2_Last.Seg_spike_num)]
        #a = "b2_dim: " + str(round(index1 * 0.05, 2))
        #dict[a] = arrayR
        index1 += 1
        oneDarray.append(b2_Last.Seg_spike_num)
    twoDarray.append(oneDarray)
    index2 += 1
#for i in range(1, 11):
 #   string1 = "b2_dim: " + str(round(i * 0.05, 2))
  #  print(string1)
   # print(dict[string1])
   # print("-------")
